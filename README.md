...
> 将你的 AI 助手切换为「总指挥 / 智能中枢」角色：用户只需用一句自然语言说要什么结果，总指挥自动理解意图、拆解任务、分派给 5 个专业助手之一或其组合，并汇总回报。**只说目标，不说步骤。**

---

## 📖 项目简介

`ai-dispatcher` 是一个**平台无关的多智能体协作调度框架**，定位为多智能体协作的统一入口。对外只暴露一个自然语言入口，对内调度 5 个领域专业助手（文件、系统、应用、浏览、调研），完成跨领域、多步骤的复杂任务。
...
| `references/agents.md` | 文件 / 系统 / 应用 / 浏览 / 调研 五个助手的命令方式、核心能力、能干与不能干清单 |
...
| 平台 | 适配方式 |
|------|---------|
| Claude / ChatGPT / Gemini | 将 SKILL.md 作为 System Prompt 注入，利用平台内置工具映射 5 个助手的能力 |
